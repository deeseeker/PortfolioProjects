...
def LOGGER(event, filename, ip, client, type, data):
    ...
    with open(os.path.join(log_directory+'/', log_prefix+filename), 'a+') as log:
        if (event == 'info'):
...
#create method to verify is IMIE is valid
...
def read_incoming_packet(client, packet):
    ...
    if (protocol_name == 'login'):
        ...
        if (packet_list[0] == '06'): 
            data ={
                "imie":json.dumps(addresses[client]['imei']),
                "software_version":int(packet_list[3], base=16),
                "battery":int(packet_list[2], base=16),
                "upload_interval":int(packet_list[4], base=16),
                "signal_strength":''
            }
            store_event('status',json.dumps(data),addresses[client]['imei'])
            ...
            data ={
                ...
                "battery":int(packet_list[2], base=16),
                "upload_interval":int(packet_list[4], base=16),
                "signal_strength":int(packet_list[5], base=16)
            }
            store_event('status',json.dumps(data),addresses[client]['imei'])        
...
def answer_login(client, query):
    ...
    data ={
        "imie":json.dumps(addresses[client]['imei']),
        "software_version":json.dumps(addresses[client]['software_version'])
    }
    store_event('login',json.dumps(data),addresses[client]['imei'])
...
def answer_gps(client, query):
    ...
    data ={
            "address":addresses[client]['address'][0],
            "imie":addresses[client]['imei'],
            "software_version":addresses[client]['software_version'],
            "gps":positions[client]['gps']
        }
    store_event('gps',json.dumps(data) ,addresses[client]['imei'])
...
# GMAPS_API_KEY = 'AIzaSyDM7w5XhFMQ0Mw7Ht-1tNxkFQumI1c87ws'
